...
namespace FIMSpace.AnimationTools
{
    ...
    public class ADBoneReference
    {
        ...
        internal void BakeCurrentState(ADArmatureSetup armature, float keyTime)
        {
            ...
            if (human)
            {
                ...
                if (IsIKElement)
                {
                    ...
                    ikPos = bodySpaceRot * (ikPos - armature.bake.bodyPosition);
        ...
        public static void WrapBake(AnimationCurve curve, bool useAdditionalFrames = true)
        {
            ...
            //WrapKeys(curve, 0, curve.keys.Length - 1, 0f, false, true);
            WrapKeys(curve, 0, curve.keys.Length - 1, 0.5f, true, true);
            //WrapKeys(curve, 0, curve.keys.Length - 1, 1f, false, true);
            //WrapKeys(curve, 0, curve.keys.Length - 1, 0.1f, true, false);

            if (useAdditionalFrames == false) return;
            if (LoopWrapAdditionalFrames == 0) return;
            if (curve.keys.Length < 12) return;
        ...
        internal void SaveCurvesForClip(ref AnimationClip clip, float reduction, bool animType = false)
        {
